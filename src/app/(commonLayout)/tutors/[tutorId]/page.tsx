...
export default function TutorDetailPage() {
  ...
  const reviewCount = tutor?.reviewCount ?? tutor?.reviews?.length ?? 0;

  const handleBook = (availabilityId: string) => {
    router.push(`/bookings/new?tutorId=${tutorId}&availabilityId=${availabilityId}`);
  };

  // FIX: Always render layout wrapper, handle all states inside
  return (
    ...
    <Loader2 className="h-12 w-12 animate-spin text-primary" />
          ...
          <div className="fixed inset-0 flex items-center justify-center text-center p-4">
            <div>
              ...
              <Button onClick={() => router.push('/tutors')}>Browse All Tutors</Button>
            </div>
          </div>
        ) : (
          <>
              ...
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row gap-6 items-start">
                  ...
                  <div className="text-right sm:ml-auto">
                    ...
                    <div className="flex items-center gap-1 mt-2 text-sm justify-end">
